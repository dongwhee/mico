...
Claude Code를 plan-only 오케스트레이터(기본 Opus, Opus advisor 페어링)로 띄우고, 실제 작업은 전문
서브에이전트(implementer·web-researcher·code-investigator·git-runner·lightweight-runner)에
...
```bash
mico                          # Opus 오케스트레이터(effort xhigh + Opus advisor) + Sonnet 5 implementer
mico --orch sonnet            # 가벼운 오케스트레이터: Sonnet 5 실행 + Opus advisor
