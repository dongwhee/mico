...
Claude Code를 plan-only 오케스트레이터(기본 Sonnet 5, Opus advisor 페어링)로 띄우고, 실제 작업은 전문
...
```bash
mico                          # Sonnet 5 오케스트레이터(effort xhigh + Opus advisor) + implementer(잘 정의된 작업은 Sonnet 5, 어렵거나 설계 판단이 필요한 작업은 Opus)
