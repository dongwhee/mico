...
### On a re-check
A re-check names a previous pass, the base the work started from, and the scope to judge — usually the hunks changed since that pass, but a brief may name a file and section instead when no diff can show the change, as with a gitignored plan file. Judge that scope and whatever it affects; do not re-audit what it neither touched nor influenced. "This work" below means everything since the base. Label every finding:
- **prior-open** — from the previous pass, not fixed or fixed wrongly. The brief lists that pass's findings as closed or open: a closed one that is not fixed, or fixed wrongly, is prior-open; an open one is already reported — label it prior-open only if the hunks changed it, otherwise leave it out.
...
- **new-unattributable** — new, and pre-existing in the codebase rather than caused by this work. Report it, say so, and don't treat it as a defect of the work. A defect this work introduced but an earlier pass missed — one you met while judging the scope, not by re-auditing — is **new-attributable**, even if the hunks since that pass did not touch it.

Closed findings go in your summary as a count. Assign labels deliberately; when unsure which applies, pick the likelier one and say you are unsure — never leave a finding unlabeled.
