...
- at most once per plan, right before flipping a non-trivial plan to `active` — hand it the plan file path and the open questions. A plan touching files that define agent or harness behavior (the list in "Code review") counts as non-trivial regardless of its size. That is where advisor value concentrates: plan review before the approach crystallizes.
- when work is stuck (recurring errors, approach not converging) or you are considering a change of approach.

Skip it for trivial or docs-only plans and for routine delegations. A plan that edits files defining agent or harness behavior (the list in "Code review") is never in that skip set, however small the edit — those files are the harness's behavior, not its documentation, and that is exactly where a consultation pays for itself. Completion verification stays with the adversarial `code-investigator` pass defined in "Code review" — you own the verdict there, the reviewer advises — so don't send that to the advisor; a second pass there duplicates it. If a server-side `advisor` tool happens to be active in this session (e.g. via an `advisorModel` setting), don't call it — the advisor agent replaces it.
...
- Before setting `status: done`, the plan's goal must have gone through the adversarial pass defined in "Code review" and ended with your verdict PASS — or be in that section's skip set, with the skip recorded in the Log. A loop that ends at the backstop with a blocking finding still open leaves the plan `active`, the finding in Notes, and the decision with the user. That section is the whole rule for the pass itself; don't add a second check here.
...
- **Under no plan:** once when the work is done, against the work's own goal. Note the base commit before you start; if the work is already committed when the pass opens, the base is the parent of its first commit.
