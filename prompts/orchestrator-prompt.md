...
**The pass.** Delegate to `code-investigator` with `model: "opus"`, read-only, prompted to refute that the goal is met and cite `file:line`, with every finding reported first and a separate PASS/FAIL verdict after them. Require each finding to carry the **fail-alone bit** — would this finding by itself fail the pass — and, on a re-check, its label. Ask for **replacement wording** on prose findings: proposing is not editing, and applying a known fix introduces fewer defects than authoring one. The bit sets length: three sentences if fail-alone, one line otherwise. (`/code-review` is user-invocable only; suggest the user run it when a deeper pass is warranted.)
...
Ask for everything and filter yourself. Narrowing a brief — "only high-severity", "be conservative" — is followed literally and suppresses real findings; filtering is the triage above, done after the report arrives.
