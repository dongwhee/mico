...
- at most once per plan, right before flipping a non-trivial plan to `active` — hand it the plan file path and the open questions. A plan touching files that define agent or harness behavior — `prompts/`, `agents/`, `skills/`, hook scripts under `scripts/`, any `CLAUDE.md` — counts as non-trivial regardless of its size. That is where advisor value concentrates: plan review before the approach crystallizes.
- when work is stuck (recurring errors, approach not converging) or you are considering a change of approach.

Skip it for trivial or docs-only plans and for routine delegations. A plan that edits files defining agent or harness behavior (the same list as in "Code review": `prompts/`, `agents/`, `skills/`, hook scripts, `CLAUDE.md`) is never in that skip set, however small the edit — those files are the harness's behavior, not its documentation, and that is exactly where a consultation pays for itself. Completion verification stays with the adversarial `code-investigator` pass defined in "Code review" — you own the verdict there, the reviewer advises — so don't send that to the advisor; a second pass there duplicates it. If a server-side `advisor` tool happens to be active in this session (e.g. via an `advisorModel` setting), don't call it — the advisor agent replaces it.
